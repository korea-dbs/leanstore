...
ResidentPageSet::ResidentPageSet(u64 max_count)
    : capacity_(max_count), count_(std::bit_ceil(static_cast<u64>(capacity_ * 1.5))), mask_(count_ - 1), clock_pos_(0) {
  spdlog::debug("Initialize replacer with %lu slots", count_);
...
ResidentPageSet::~ResidentPageSet() {
  munmap(slots_, count_ * sizeof(Entry));
...
void ResidentPageSet::Insert(pageid_t page_id) {
  ...
  while (true) {
    ...
    if ((curr == ResidentPageSet::EMPTY) || (curr == ResidentPageSet::TOMBSTONE)) {
      if (slots_[pos].pid.compare_exchange_weak(curr, page_id)) { return; }
    }
...
auto ResidentPageSet::Remove(pageid_t page_id) -> bool {
  ...
  while (true) {
    ...
    if (curr == page_id) {
      if (slots_[pos].pid.compare_exchange_weak(curr, ResidentPageSet::TOMBSTONE)) { return true; }
    }
...
void ResidentPageSet::IterateClockBatch(u64 batch, const std::function<void(pageid_t)> &evict_fn) {
  ...
  } while (!clock_pos_.compare_exchange_weak(pos, new_position));
