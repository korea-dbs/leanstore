...
auto AliasingArea::RequestAliasingArea(u64 requested_size) -> storage::Page * {
  ...
  if (ALIAS_AREA_CAPABLE(local_area_ptr_[LeanStore::worker_thread_id] + required_page_cnt,
                         LeanStore::worker_thread_id)) {
    auto curr_index = local_area_ptr_[LeanStore::worker_thread_id];
    local_area_ptr_[LeanStore::worker_thread_id] += required_page_cnt;
  ...
  while (true) {
    ...
    if (acquire_success) {
      acquired_locks_[LeanStore::worker_thread_id].emplace_back(pos, required_block_cnt);
...
void AliasingArea::ReleaseAliasingArea() {
  if (local_area_ptr_[LeanStore::worker_thread_id] > ALIAS_LOCAL_PTR_START(LeanStore::worker_thread_id)) {
    local_area_ptr_[LeanStore::worker_thread_id] = ALIAS_LOCAL_PTR_START(LeanStore::worker_thread_id);
  }
  if (!acquired_locks_[LeanStore::worker_thread_id].empty()) {
    for (auto &[block_pos, block_cnt] : acquired_locks_[LeanStore::worker_thread_id]) {
      Ensure(ToggleShalasLocks(false, block_pos, block_pos + block_cnt));
    }
    acquired_locks_[LeanStore::worker_thread_id].clear();
  }
}
