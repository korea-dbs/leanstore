...
class TestAioInterface : public BaseTest {
 protected:
  ...
  void SetUp() override {
    ...
    spdlog::info("Alloc size %lu", ALLOC_SIZE);
...
TEST_F(TestAioInterface, BasicTest) {
  ...
  int ret      = io_uring_queue_init(8, &(aio_inf.Ring()), storage::LibaioInterface::uring_flags);
  if (ret != 0) { throw ex::EnsureFailed("TestAioInterface: io_uring_queue_init error"); }
