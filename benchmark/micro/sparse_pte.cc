...
int main() {
  ...
  spdlog::info("Page table size %s - tier vitr size %llu", leanstore::PageTableSize().c_str(), tier_virtual_size);

  for (auto idx = 0ULL; idx < tier_virtual_size; idx++) { bm[idx] = 1; }
  spdlog::info("Page table size %s", leanstore::PageTableSize().c_str());
  ...
  for (int blob = 0; blob <= 10000; blob++) {
    ...
    if (blob % 1000 == 0) {
      spdlog::info("After Blob %d, Page table size %s", blob, leanstore::PageTableSize().c_str());
    }
  }

  // Now delete ~90% of all BLOBs
  spdlog::info("De-allocation");
  for (int blob = 0; blob <= 10000; blob++) {
    if (rand() % 10 < 9) {
