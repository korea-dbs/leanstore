...
auto main(int argc, char **argv) -> int {
  ...
  if (std::find(std::begin(ycsb::SUPPORTED_PAYLOAD_SIZE), std::end(ycsb::SUPPORTED_PAYLOAD_SIZE),
    ...
    spdlog::error("Payload size %lu not supported, check ycsb::SUPPORTED_PAYLOAD_SIZE", FLAGS_ycsb_payload_size);
    return 0;
  }
  ...
  for (auto &payload : payloads) {
    payload = leanstore::AlignedPtr(std::max(GLOBAL_BLOCK_SIZE, FLAGS_ycsb_max_payload_size));
  }
  ...
  db->Shutdown();
  spdlog::info("Space used: {:.4f} GB - WAL size: {:.4f} GB", db->AllocatedSize(), db->WALSize());
  e.stopCounters();
