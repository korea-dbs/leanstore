...
auto main(int argc, char **argv) -> int {
  ...
  spdlog::info("Complete loading. Off-row records: %u - In-row records: %u", wiki->offrow_count.load(),
               wiki->inrow_count.load());
  spdlog::info("Time: %lu ms - Space used: %.4f GB",
               std::chrono::duration_cast<std::chrono::milliseconds>(time_end - time_start).count(),
               db->AllocatedSize());
#ifdef DEBUG
  db->worker_pool.ScheduleSyncJob(0, [&]() {
    db->StartTransaction();
    spdlog::info("Record count: %lu", wiki->CountEntries());
  ...
  db->Shutdown();
  spdlog::info("Total completed txn %lu - Space used: %.4f GB - WAL size: %.4f GB",
