...
auto main(int argc, char **argv) -> int {
  ...
  spdlog::info("Start loading initial data");
  ...
  wiki_relation.MiniTransactionWrapper([&]() {
    spdlog::info("Record count: %lu - No articles: %lu", wiki_relation.Count(), articles.size());
    assert(wiki_relation.Count() == articles.size());
  });
  spdlog::info("Complete loading. Time: %lu ms - Space used: %.4f GB",
  ...
  if (FLAGS_blob_indexing_variant == 1) {
    // Index construction
    spdlog::info("Start building Blob Handler Index");
        ...
        if (FLAGS_wiki_index_evaluate_deduplication) {
          ...
          [[maybe_unused]] auto found = bh_index.LookUpBlob(blob, [&](const auto &rec) {
            if (key.my_key != rec.article_id) {
              spdlog::error("Duplication: Line %lu and %lu", key.my_key, rec.article_id);
            }
          });
          assert(found);
        }
    ...
    spdlog::info("Blob Handler Index build complete. Time: %lu ms - Space used: %.4f MB",
  ...
  if (FLAGS_blob_indexing_variant == 2) {
    auto rejection = 0;
    // Index construction
    spdlog::info("Start building Prefix Index");
    time_start = std::chrono::high_resolution_clock::now();
    wiki_relation.MiniTransactionWrapper([&]() {
      wiki_relation.Scan({0}, [&](const BlobRelation::Key &key, const BlobRelation &record) {
    ...
    spdlog::info("Prefix Index build complete. Time: %lu ms - Space used: %.4f MB",
                 std::chrono::duration_cast<std::chrono::milliseconds>(time_end - time_start).count(),
                 prefix_index_size);
    if (FLAGS_wiki_index_evaluate_deduplication) { spdlog::info("Prefix Index: Reject %u records", rejection); }
  }
  ...
  db->Shutdown();
  spdlog::info("Total completed txn %lu - Space used: %.4f GB - WAL size: %.4f GB - Not found items %lu",
               leanstore::statistics::total_committed_txn.load(), db->AllocatedSize(), db->WALSize(), not_found);
  e.stopCounters();
