...
void TATPWorkload<AdapterType>::LoadSubcribers() {
  spdlog::debug("Load %u subcribers", num_subcribers);
  ...
  spdlog::debug("Load subcribers successfully");
}
...
template <template <typename> class AdapterType>
auto TATPWorkload<AdapterType>::GetNewDestination(UniqueID s_id, uint8_t sf_type, uint8_t start_time, uint8_t end_time)
  -> ResultCode {
...
template <template <typename> class AdapterType>
auto TATPWorkload<AdapterType>::UpdateSubcriberData(UniqueID s_id, bool bit_1, uint8_t sf_type, uint8_t data_a)
  -> ResultCode {
...
template <template <typename> class AdapterType>
auto TATPWorkload<AdapterType>::DeleteCallForwarding(const Varchar<15> &sub_nbr, uint8_t sf_type, uint8_t start_time)
  -> ResultCode {
  // Find s_id
  UniqueID s_id;
  auto found = subcriber_nbr.LookUp({sub_nbr}, [&](const SubscriberNbrIndex &rec) { s_id = rec.s_id; });
