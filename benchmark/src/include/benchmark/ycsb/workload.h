...
using WorkerLocalPayloads = std::vector<std::unique_ptr<uint8_t, leanstore::AlignedDeleter<uint8_t>>>;

class YCSBWorkloadInterface {
 public:
  static constexpr u64 PAYLOAD_ALIGNMENT = 1024;
  ...
  virtual auto NextTransactionArrivalTime(const std::function<void()> &idle_fn) -> uint64_t = 0;

  static auto PayloadSize() -> uint64_t {
    if (FLAGS_ycsb_random_payload) {
      return RoundUp(PAYLOAD_ALIGNMENT,
...
struct YCSBWorkloadNoBlobRep : public YCSBWorkloadInterface {
  ...
  const Integer read_ratio;    // Read ratio

  template <typename... Params>
  YCSBWorkloadNoBlobRep(Integer initial_record_cnt, Integer required_read_ratio, double zipf_theta,
  ...
  void LoadInitialData(UInteger w_id, const tbb::blocked_range<Integer> &range) override {
    ...
    for (auto key = range.begin(); key < range.end(); key++) {
      ...
      RandomGenerator::GetRandRepetitiveString(reinterpret_cast<uint8_t *>(record.payload.data), 128UL, payload_sz);
      relation.Insert({r_key}, record);
    }
  }

  auto NextTransactionArrivalTime([[maybe_unused]] const std::function<void()> &idle_fn) -> uint64_t override {
    return 0;
  }

  void ExecuteTransaction(UInteger w_id) override {
    auto access_key  = static_cast<UInteger>(zipf_generator.Rand());
    auto is_read_txn = UniformRand(1, 100) <= read_ratio;
    auto payload     = payloads[w_id].get();
    auto record      = reinterpret_cast<YCSBRelation *>(payload);
...
struct YCSBWorkload : public YCSBWorkloadInterface {
  ...
  template <typename... Params>
  YCSBWorkload(Integer initial_record_cnt, Integer required_read_ratio, double zipf_theta, bool use_blob_register,
               double txn_rate_per_worker, WorkerLocalPayloads &payloads, Params &&...params)
      : relation(AdapterType<YCSBRelation>(std::forward<Params>(params)...)),
        zipf_generator(zipf_theta, initial_record_cnt),
        payloads(std::move(payloads)),
        record_count(initial_record_cnt),
  ...
  auto NextTransactionArrivalTime(const std::function<void()> &idle_fn) -> uint64_t override {
  ...
  void ExecuteTransaction(UInteger w_id) override {
    ...
    if (!enable_blob_rep) {
      if (is_read_txn) {
        relation.LookUp({access_key}, [&](const YCSBRelation &rec) {
          std::memcpy(payload, const_cast<YCSBRelation &>(rec).payload.Data(), FLAGS_ycsb_max_payload_size);
        });
      } else {
        auto payload_sz = YCSBWorkloadInterface::PayloadSize();
        RandomGenerator::GetRandRepetitiveString(payload, 128UL, payload_sz);
        relation.UpdateInPlace({access_key},
                               [&](YCSBRelation &rec) { std::memcpy(rec.payload.Data(), payload, payload_sz); });
      }
      return;
    }
    ...
    if (FLAGS_ycsb_benchmark_fstat) {
      ...
      relation.Scan({access_key}, [&](const auto &, const YCSBRelation &rec) {
        ...
        std::memcpy(blob_rep, const_cast<YCSBRelation &>(rec).payload.Data(), rec.PayloadSize());
        return (++cnt < 10);
      });
      return;
    }
    ...
    if (is_read_txn) {
      ...
      if (new_blob_rep.size() == blob_rep_size) [[likely]] {
        relation.UpdateInPlace({access_key}, [&](YCSBRelation &rec) {
          std::memcpy(blob_rep, const_cast<YCSBRelation &>(rec).payload.Data(), rec.PayloadSize());
          std::memcpy(rec.payload.Data(), new_blob_rep.data(), new_blob_rep.size());
        });
      } else {
        relation.UpdateRawPayload({access_key}, new_blob_rep, [&](const YCSBRelation &prev) {
          std::memcpy(blob_rep, const_cast<YCSBRelation &>(prev).payload.Data(), prev.PayloadSize());
        });
      }
...
using YCSBKeyValue  = ycsb::YCSBWorkload<LeanStoreAdapter, ycsb::Relation<BytesPayload<ycsb::BLOB_NORMAL_PAYLOAD>, 0>>;
using YCSBBlobState = ycsb::YCSBWorkload<LeanStoreAdapter, ycsb::BlobStateRelation>;
