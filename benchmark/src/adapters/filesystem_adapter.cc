...
void FilesystemAsDB::DropCache() {
  ...
  assert(ret >= 0);
  spdlog::info("Complete dropping page cache");
}
...
thread_local leanstore::SHA256H FilesystemAdapter<RecordBase>::sha_context = {};
...
void FilesystemAdapter<RecordBase>::ExtraOperator(std::span<const uint8_t> payload) {
  uint8_t sha2_digest[leanstore::SHA256H::DIGEST_SIZE];
...
template <class RecordBase>
auto FilesystemAdapter<RecordBase>::UpdateInPlace(const typename RecordBase::Key &r_key,
