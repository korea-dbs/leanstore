...
class AtomicMarkableReference {
 ...
 public:
  ...
  AtomicMarkableReference &operator=(AtomicMarkableReference &&other) noexcept;
  void store(T *ref, bool mark);
  bool compareAndSet(T *expectedRef, T *newRef, bool expectedMark, bool newMark);
...
template <typename T>
AtomicMarkableReference<T>::AtomicMarkableReference(AtomicMarkableReference &&other) noexcept {
  atomicRefMark.store(other.atomicRefMark.load(std::memory_order_relaxed), std::memory_order_relaxed);
}

template <typename T>
AtomicMarkableReference<T> &AtomicMarkableReference<T>::operator=(AtomicMarkableReference &&other) noexcept {
  if (this != &other) {
    atomicRefMark.store(other.atomicRefMark.load(std::memory_order_relaxed), std::memory_order_relaxed);
  }
  return *this;
...
template <typename T>
bool AtomicMarkableReference<T>::compareAndSet(T *expectedRef, T *newRef, bool expectedMark, bool newMark) {
  uintptr_t expected = pack(expectedRef, expectedMark);
  uintptr_t desired  = pack(newRef, newMark);
  return atomicRefMark.compare_exchange_strong(expected, desired, std::memory_order_acq_rel);
}
...
bool AtomicMarkableReference<T>::attemptMark(T *expectedRef, bool newMark) {
  ...
  if (currentRef == expectedRef && currentMark != newMark) {
    ...
    return atomicRefMark.compare_exchange_strong(current, desired, std::memory_order_acq_rel);
  }
  return false;
}
